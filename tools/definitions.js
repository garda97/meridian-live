...
2. Screening auto_strategy deploy_plan on candidate block → follow exactly (strategy + bins_below + bins_above)
...
- spot/curve + SOL only: may use bins_above>0 for balanced range (total bins >= 35).
- Do not deploy candidates marked entry_gate: BLOCK.

Guidelines (only when user hasn't specified and no auto_strategy block):
- Strategy: omit to use config default, or use auto_strategy from screening candidate
- Deposit: SOL only via amount_y/amount_sol, amount_x=0
      ...
      parameters: {
        ...
        properties: {
          ...
          strategy: {
            ...
            description: "DLMM strategy: bid_ask (dump/retracement), spot (sideways), curve (flat range). Use auto_strategy from screening candidate when present."
          },
          bins_below: {
            type: "number",
