...
switch (subcommand) {
    ...
    for (const pool of pools) {
      ...
      enriched.push({
        ...
        token: {
          ...
          audit: {
            top10_pct: ti?.audit?.gmgn_top10_pct ?? ti?.audit?.top_holders_pct,
            bots_pct: ti?.audit?.bot_holders_pct,
            gmgn_mint_renounced: ti?.audit?.gmgn_mint_renounced,
            gmgn_freeze_renounced: ti?.audit?.gmgn_freeze_renounced,
            gmgn_burn_status: ti?.audit?.gmgn_burn_status,
          },
        },
  ...
  case "helius-keys": {
    ...
    reloadHeliusKeys();
    if (action === "status") {
      out(getRotatorStatus());
      break;
    }
    if (action === "rotate") {
      const { rotateHeliusKey } = await import("./utils/helius-rotator.js");
      const next = rotateHeliusKey("manual_cli");
      out({ rotated_to: `${next.slice(0, 8)}…`, status: getRotatorStatus() });
      break;
    }
    die("Usage: meridian helius-keys [status|rotate] — full CLI: node scripts/helius_keys.js");
  }
